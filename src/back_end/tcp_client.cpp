...
void
TcpClient::Init(const std::string& _ip, const int _port)
{
  ip_ = _ip;
  port_ = _port;
...
void
TcpClient::Connect(const std::string& _ip, const int _port)
{
  Init(_ip, _port);
  Connect();
}
...
TcpClient::Connect()
{
  ...
  if (connect(sock_fd_, (struct sockaddr*)&serv_addr_, sizeof(serv_addr_)) <
      0) {
