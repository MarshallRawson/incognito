...
TcpServer::TcpServer(int _port) : port_(_port)
{
  ...
  opt_ = 1;
  if (setsockopt(sock_fd_, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt_, sizeof(opt_)) != 0)
  {
...
void TcpServer::Launch()
{
  ...
  while(true)
  {
    int conn_fd = accept(sock_fd_, (struct sockaddr *)&address_, (socklen_t*)&addrlen_);
